...
if (isset($_POST['submit']) && isset($_POST['newpassword'])) {
    if (empty($_POST['oldpassword']) || empty($_POST['newpassword'])) {
        ...
        $result = $db->mGetRow("itv_admin", "*", "where name='$username' and psw='$oldpassword'");
        if ($result) {
            $db->mSet("itv_admin", "psw='$newpassword'", "where name='$username'");
...
if (isset($_POST['submit']) && isset($_POST['newsecret_key'])) {
    if (empty($_POST['newsecret_key']) || empty($_POST['newsecret_key_confirm'])) {
        ...
        if ($newsecret_key_input == $newsecret_key_confirm) {
            ...
            $db->mSet("itv_config", "value='$newsecret_key'", "where name='secret_key'");
...
if (isset($_POST['closesecret_key'])) {
    $db->mSet("itv_config", "value=NULL", "where name='secret_key'");
...
if (isset($_POST['deleteadmin'])) {
    if (empty($_POST['adminname'])) {
        ...
        foreach ($_POST['adminname'] as $name) {
            if ($name <> 'admin') {
                $db->mDel("itv_admin", "where name='$name'");
...
if (isset($_POST['saveauthorinfo'])) {
    if (!empty($_POST['adminname'])) {
        $db->mSet("itv_admin", "author=0,useradmin=0,ipcheck=0,epgadmin=0,mealsadmin=0,movieadmin=0,channeladmin=0", "where name<>'admin'");
        if (!empty($_POST['author'])) {
            foreach ($_POST['author'] as $adminname) {
                $db->mSet("itv_admin", "author=1", "where name='$adminname'");
            } 
        } 
        if (!empty($_POST['useradmin'])) {
            foreach ($_POST['useradmin'] as $adminname) {
                $db->mSet("itv_admin", "useradmin=1", "where name='$adminname'");
            } 
        } 
        if (!empty($_POST['ipcheck'])) {
            foreach ($_POST['ipcheck'] as $adminname) {
                $db->mSet("itv_admin", "ipcheck=1", "where name='$adminname'");
            } 
        } 
        if (!empty($_POST['epgadmin'])) {
            foreach ($_POST['epgadmin'] as $adminname) {
                $db->mSet("itv_admin", "epgadmin=1", "where name='$adminname'");
            } 
        } 
        if (!empty($_POST['mealsadmin'])) {
            foreach ($_POST['mealsadmin'] as $adminname) {
                $db->mSet("itv_admin", "mealsadmin=1", "where name='$adminname'");
            } 
        } 
        if (!empty($_POST['movieadmin'])) {
            foreach ($_POST['movieadmin'] as $adminname) {
                $db->mSet("itv_admin", "movieadmin=1", "where name='$adminname'");
            } 
        } 
        if (!empty($_POST['channeladmin'])) {
            foreach ($_POST['channeladmin'] as $adminname) {
                $db->mSet("itv_admin", "channeladmin=1", "where name='$adminname'");
            } 
        } 
...
if (isset($_POST['submit']) && isset($_POST['appver'])) {
	...
	$db->mSet("itv_config", "value='$versionname'", "where name='appver'");
	$db->mSet("itv_config", "value='$appurl'", "where name='appurl'");
	$db->mSet("itv_config", "value='$up_size'", "where name='up_size'");
	$db->mSet("itv_config", "value='$up_sets'", "where name='up_sets'");
	$db->mSet("itv_config", "value='$up_text'", "where name='up_text'");
...
if (isset($_POST['submit']) && isset($_POST['appver_sdk14'])) {
	...
	$db->mSet("itv_config", "value='$versionname'", "where name='appver_sdk14'");
	$db->mSet("itv_config", "value='$appurl'", "where name='appurl_sdk14'");
...
if (isset($_POST['decodersel']) && isset($_POST['buffTimeOut'])) {
    ...
    if ($trialdays == 0) {
        $db->mSet("itv_users", "exp=0", "where status=-1");
    } 
	$db->mSet("itv_config", "value='$decoder'", "where name='decoder'");
	$db->mSet("itv_config", "value='$trialdays'", "where name='trialdays'");
	$db->mSet("itv_config", "value='$buffTimeOut'", "where name='buffTimeOut'");
    echo"<script>showindex=4;lightyear.notify('设置成功！', 'success', 3000);</script>";
} 

if (isset($_POST['submitsetver'])) {
	$db->mSet("itv_config", "value=value+1", "where name='setver'");
...
if (isset($_POST['submittipset'])) {
	...
	$db->mSet("itv_config", "value='$tiploading'", "where name='tiploading'");
	$db->mSet("itv_config", "value='$tipusernoreg'", "where name='tipusernoreg'");
	$db->mSet("itv_config", "value='$tipuserexpired'", "where name='tipuserexpired'");
	$db->mSet("itv_config", "value='$tipuserforbidden'", "where name='tipuserforbidden'");
...
if (isset($_POST['weaapi_id']) && isset($_POST['weaapi_key'])) {
    ...
    if (empty($weaapi_id)) {
        ...
        $db->mSet("itv_config", "value='$showwea'", "where name='showwea'");
        $db->mSet("itv_config", "value='$weaapi_id'", "where name='weaapi_id'");
        $db->mSet("itv_config", "value='$weaapi_key'", "where name='weaapi_key'");
        if ($showwea == 0) {
...
if (isset($_POST['submit']) && isset($_POST['adtext'])) {
    ...
    $adinfo = $_POST['adinfo'];
	$db->mSet("itv_config", "value='$adinfo'", "where name='adinfo'");
	$db->mSet("itv_config", "value='$adtext'", "where name='adtext'");
	$db->mSet("itv_config", "value='$showtime'", "where name='showtime'");
	$db->mSet("itv_config", "value='$showinterval'", "where name='showinterval'");
    echo"<script>showindex=0;lightyear.notify('公告修改成功！', 'success', 3000);</script>";
} 

if (isset($_POST['submit']) && isset($_POST['epgurl'])) {
    $epgurl = $_POST['epgurl'];
	$tmua = $_POST['tmua'];
	$db->mSet("itv_config", "value='$epgurl'", "where name='epgurl'");
	$db->mSet("itv_config", "value='$tmua'", "where name='tmua'");
    echo"<script>showindex=0;lightyear.notify('EPG和UA设置成功！', 'success', 3000);</script>";
} 

if (isset($_POST['submit']) && isset($_POST['imgstart'])) {
    $imgstart = $_POST['imgstart'];
	$imgend = $_POST['imgend'];
	$db->mSet("itv_config", "value='$imgstart'", "where name='imgstart'");
	$db->mSet("itv_config", "value='$imgend'", "where name='imgend'");
    echo"<script>showindex=0;lightyear.notify('背景图设置成功！', 'success', 3000);</script>";
} 
...
if (isset($_POST['submitappinfo'])) {
    ...
    $db->mSet("itv_config", "value='$app_sign'", "where name='app_sign'");
    $db->mSet("itv_config", "value='$app_appname'", "where name='app_appname'");
    $db->mSet("itv_config", "value='$app_packagename'", "where name='app_packagename'");
...
if (isset($_POST['alipay_set'])) {
	...
	$db->mSet("itv_config", "value='$alipay_appid'", "where name='alipay_appid'");
	$db->mSet("itv_config", "value='$alipay_publickey'", "where name='alipay_publickey'");
	$db->mSet("itv_config", "value='$alipay_privatekey'", "where name='alipay_privatekey'");
...
if (isset($_POST['submitauthor'])) {
    ...
    $db->mSet("itv_config", "value='$needauthor'", "where name='needauthor'");
} 

if (isset($_POST['clearlog'])) {
    $db->mDel("itv_adminrec");
    echo"<script>showindex=2;lightyear.notify('后台记录已清空！', 'success', 3000);</script>";
} 

if (isset($_POST['ipchk'])) {
    $ipchk = $_POST['ipchk'];
    $db->mSet("itv_config", "value='$ipchk'", "where name='ipchk'");
...
$dataver = $db->mGet("itv_config", "value", "where name='dataver'");
$appver = $db->mGet("itv_config", "value", "where name='appver'");
$appver_sdk14 = $db->mGet("itv_config", "value", "where name='appver_sdk14'");
$setver = $db->mGet("itv_config", "value", "where name='setver'");
$dataurl = $db->mGet("itv_config", "value", "where name='dataurl'");
$appurl = $db->mGet("itv_config", "value", "where name='appurl'");
$appurl_sdk14 = $db->mGet("itv_config", "value", "where name='appurl_sdk14'");
$showtime = $db->mGet("itv_config", "value", "where name='showtime'");
$showinterval = $db->mGet("itv_config", "value", "where name='showinterval'");
$splash = $db->mGet("itv_config", "value", "where name='splash'");
$needauthor = $db->mGet("itv_config", "value", "where name='needauthor'");
$decoder = $db->mGet("itv_config", "value", "where name='decoder'");
$buffTimeOut = $db->mGet("itv_config", "value", "where name='buffTimeOut'");
$tiploading = $db->mGet("itv_config", "value", "where name='tiploading'");
$tipusernoreg = $db->mGet("itv_config", "value", "where name='tipusernoreg'");
$tipuserexpired = $db->mGet("itv_config", "value", "where name='tipuserexpired'");
$tipuserforbidden = $db->mGet("itv_config", "value", "where name='tipuserforbidden'");
$trialdays = $db->mGet("itv_config", "value", "where name='trialdays'");
...
$secret_key = $db->mGet("itv_config", "value", "where name='secret_key'");
$weaapi_id = $db->mGet("itv_config", "value", "where name='weaapi_id'");
$weaapi_key = $db->mGet("itv_config", "value", "where name='weaapi_key'");
$app_sign = $db->mGet("itv_config", "value", "where name='app_sign'");
$app_appname = $db->mGet("itv_config", "value", "where name='app_appname'");
$app_packagename = $db->mGet("itv_config", "value", "where name='app_packagename'");
$alipay_appid = $db->mGet("itv_config", "value", "where name='alipay_appid'");
$alipay_publickey = $db->mGet("itv_config", "value", "where name='alipay_publickey'");
$alipay_privatekey = $db->mGet("itv_config", "value", "where name='alipay_privatekey'");
$ipchk = $db->mGet("itv_config", "value", "where name='ipchk'");
