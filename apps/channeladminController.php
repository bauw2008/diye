...
function sort_id() {
    ...
    $result = $db->mQuery("SELECT * from itv_category where type='$categorytype' order by id");
    while ($row = mysqli_fetch_array($result)) {
        $name = $row['name'];
        $db->mSet("itv_category", "id=$numCount", "where name='$name'");
        unset($name);
        $numCount++;
    } 
    unset($row);
...
function chk_sort_id() {
    ...
    if ($row = $db->mGetRow("itv_category", "min(id),max(id)", "where type='$categorytype'")) {
        $minid = $row['min(id)'];
        $maxid = $row['max(id)'];
    } 
} 
...
function add_channel_list($cname, $srclist) {
    ...
    if (!empty($srclist && $cname)) {
        $db->mDel("itv_channels", "where category='$cname'");
        $repetnum = 0;
        $rows = explode("\n", $srclist);
            ...
            if (strpos($row, ',') !== false) {
                ...
                if (strpos($source, '#') !== false) {
                    ...
                    foreach ($sources as $src) {
                        ...
                        $src2 = str_replace("}", "", $src2);
                        $src2 = str_replace("{", "", $src2);
                            $channelurl = $db->mQuery("SELECT url from itv_channels where category='$cname");
                        ...
                        if ($channelname != '' && $src2 != '') {
                            $db->mInt("itv_channels", "id,name,url,category", "NULL,'$channelname','$src2','$cname'");
                        } 
                    } 
                } else {
...
if (isset($_GET['category'])) {
    ...
    if ($row = $db->mGetRow("itv_category", "name", "order by id")) {
        $cname = $row['name'];
        unset($row);
    } else {
        $cname = '';
...
if (isset($_POST['submit']) && isset($_POST['category']) && isset($_POST['srclist'])) {
    ...
    if ($listreturn != -1) {
     // echo"<script>showindex=$showindex;lightyear.notify('保存成功！$listreturn 个重复节目源！', 'success', 3000);</script>";
        echo"<script>showindex=$showindex;lightyear.notify('保存成功！', 'success', 3000);</script>";
    } else {
...
if (isset($_POST['submit']) && isset($_POST['category'])) {
    ...
    if ($category == "") {
        ...
        $numCount = $maxid + 1;
        $categoryname = $db->mGetRow("itv_category", "name", "where name='$category'");
        if (empty($categoryname)) {
            $db->mInt("itv_category", "id,name,psw,type", "$numCount,'$category','$cpass','$categorytype'");
            $showindex = $db->mGet("itv_category", "count(*)", "where type='$categorytype'") - 1;
...
if (isset($_POST['addthirdlist'])) {
    ...
    if ($category == "") {
        ...
        $categoryname = $db->mGetRow("itv_category", "name", "where name='$category'");
        if (empty($categoryname)) {
            $db->mInt("itv_category", "id,name,psw,type,url", "$numCount,'$category','$cpass','$categorytype','$listurl'");
            $showindex = $db->mGet("itv_category", "count(*)", "where type='$categorytype'") - 1;
            ...
            if ($addlist !== -1) {
                ...
                $db->mDel("itv_category", "where name='$category'");
            } 
            sort_id();
        } 
    } 
...
if (isset($_POST['updatelist'])) {
	...
	$listurl=$db->mGet("itv_category", "url", "where name='$category'");
    $srclist = file_get_contents($listurl);
    if ($category == "") {
        echo "<script>lightyear.notify('列表名称不能为空！', 'danger', 3000);</script>";
    } else {
        $listurl = $db->mGetRow("itv_category", "url", "where name='$category'");
        $addlist = add_channel_list($category, $srclist);
        if ($addlist !== -1) {
...
if (isset($_POST['submit_deltype']) && isset($_POST['category'])) {
    ...
    if ($category == "") {
        ...
        if ($categoryid = $db->mGet("itv_category", "id", "where name='$category'")) {
            $db->mSet("itv_category", "id=id-1", "where id>$categoryid");
        } 
        $db->mDel("itv_category", "where name='$category'");
        $db->mDel("itv_channels", "where category='$category'");
        sort_id();
...
if (isset($_POST['submit_modifytype']) && isset($_POST['category'])) {
    ...
    if ($category == "") {
        ...
        $db->mSet("itv_category", "name='$category',psw='$cpass'", "where name='$category0'");
        $db->mSet("itv_channels", "category='$category'", "where category='$category0'");
...
if (isset($_POST['submit_moveup']) && isset($_POST['category'])) {
    $category = $_POST['category'];
    $showindex = $_POST['showindex'];
    if ($id = $db->mGet("itv_category", "id", "where name='$category'")) {
        $preid = $id-1;
        if ($preid >= $minid) {
            $db->mSet("itv_category", "id=id+1", "where id=$preid");
            $db->mSet("itv_category", "id=id-1", "where name='$category'");
...
if (isset($_POST['submit_movedown']) && isset($_POST['category'])) {
    ...
    if ($id = $db->mGet("itv_category", "id", "where name='$category'")) {
        $nextid = $id + 1;
        if ($nextid <= $maxid) {
            $db->mSet("itv_category", "id=id-1", "where id=$nextid");
            $db->mSet("itv_category", "id=id+1", "where name='$category'");
            echo "<script>showindex=$showindex+1;</script>";
        } else {
...
if (isset($_POST['submit_movetop']) && isset($_POST['category'])) {
    $category = $_POST['category'];
    if ($id = $db->mGet("itv_category", "Min(id)", "where type='$categorytype'")) {
        $id = $id-1;
        $db->mSet("itv_category", "id=$id", "where name='$category'");
        sort_id();
...
if (isset($_POST['submit']) && isset($_POST['ver'])) {
    ...
    if (isset($_POST['autoupdate'])) {
		$db->mSet("itv_config", "value='1'", "where name='autoupdate'");
		$db->mSet("itv_config", "value='$updateinterval'", "where name='updateinterval'");
    } else {
        $ver = $_POST['ver'];
		$db->mSet("itv_config", "value='0'", "where name='autoupdate'");
		$db->mSet("itv_config", "value='$ver'", "where name='dataver'");
    } 
    echo "<script>lightyear.notify('保存成功！', 'success', 3000);</script>";
} 
// 分类开关
if (isset($_POST['checkpdname'])) {
    $db->mSet("itv_category", "enable=0");
    foreach ($_POST['enable'] as $categoryenable) {
        $db->mSet("itv_category", "enable=1", "where name='$categoryenable'");
    } 
} 
// 获取列表设置
$ver = $db->mGet("itv_config", "value", "where name='dataver'");
$versionname = $db->mGet("itv_config", "value", "where name='appver'");
$autoupdate = $db->mGet("itv_config", "value", "where name='autoupdate'");
$updateinterval = $db->mGet("itv_config", "value", "where name='updateinterval'");
