...
$appid = $db->mGet("itv_config", "value", "where name='alipay_appid'");  //应用APPID
$appname = $db->mGet("itv_config", "value", "where name='app_appname'");  //应用名称
$rsaPrivateKey = $db->mGet("itv_config", "value", "where name='alipay_privatekey'");  //商户私钥

if (isset($_POST['dopay'])) {
	$userid=$_POST['userid'];
    $mealid = $_POST['mealid'];
    $mealname = $db->mGet("itv_meals", "name", "where id=$mealid");
    $amount = $db->mGet("itv_meals", "amount", "where id=$mealid");
	$days = $db->mGet("itv_meals", "days", "where id=$mealid");
    ...
    if (empty($days) || empty($mealid)) {
    	...
    	if ($db->mGetRow("itv_payment", "*", "where userid=$userid and status=0")){
    		$db->mDel("itv_payment", "where userid=$userid and status=0");
    		...
    		$db->mInt("itv_payment", "userid,order_id,meal,days,status", "$userid,'$outTradeNo',$mealid,$days,0");
    	}
    }
	$result = $aliPay->doPay();
...
if (isset($_GET['checkpay'])) {
	...
	$result = $db->mGet("itv_payment", "status", "where userid=$checkpay");
    if($result != 1){
