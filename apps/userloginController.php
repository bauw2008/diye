...
$getkey =  $db->mGet("itv_config","value","where name='secret_key'");
if (empty($getkey)) {
...
if (isset($_COOKIE['rememberpass'])) {
    ...
    if ($row = $db->mGetRow("itv_admin","*","where name='$user'")) {
        if ($psw == $row['psw']) {
            ...
            $db->mInt("itv_adminrec","id,name,ip,loc,time,func","null,'$user','$ip','$region','$time','用户登入'");
            header("location:houtai123/index.php");
        } 
    }
	unset($row);
} 
...
if (!empty($_POST['username']) && !empty($_POST['password'])) {
    ...
    if ($row = $db->mGetRow("itv_admin","*","where name='$user'")) {
        if ($psw == $row['psw']) {
            ...
            $db->mInt("itv_adminrec","id,name,ip,loc,time,func","null,'$user','$ip','$region','$time','输入错误密码'");
        } 
    } else {
        echo "<script>alert('用户不存在！');</script>";
        $db->mInt("itv_adminrec","id,name,ip,loc,time,func","null,'$user','$ip','$region','$time','尝试登陆'");
    }
	unset($row);
} 
