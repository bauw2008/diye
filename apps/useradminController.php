...
if(isset($_POST['submitclear'])){
	if (empty($_POST['id'])) {
        echo("<script>lightyear.notify('请选择要清除绑定的用户账号信息！', 'danger', 3000);</script>");
	}
	foreach ($_POST['id'] as $id) {
        $db->mSet("itv_users", "mac='',deviceid='',model=''", "where name=$id");
        $db->mDel("itv_loginrec", "where userid=$id");
	}	
    echo("<script>lightyear.notify('选中用户绑定信息已清除！', 'success', 3000);</script>");

}

if (isset($_POST['submitdelall'])) {
    $nowtime = time();
    $db->mDel("itv_users", "where status=1 and exp<$nowtime");
...
if (isset($_POST['submitmodify'])) {
    if (empty($_POST['id'])) {
        ...
        foreach ($_POST['id'] as $id) {
            $db->mSet("itv_users", "status=1,exp=$exp", "where name=$id");
...
if (isset($_POST['submitadddays'])) {
    if (empty($_POST['id'])) {
        ...
        $exp = 86400 * $_POST['exp'];
        foreach ($_POST['id'] as $id) {
            $db->mSet("itv_users", "exp=exp+$exp", "where name=$id and status=1");
...
if (isset($_POST['submitmodifymarks'])) {
    if (empty($_POST['id'])) {
        ...
        $marks = $_POST['marks'];
        foreach ($_POST['id'] as $id) {
            $db->mSet("itv_users", "marks='$marks'", "where name=$id");
...
if (isset($_POST['submitforbidden'])) {
    if (empty($_POST['id'])) {
        ...
        foreach ($_POST['id'] as $id) {
            $db->mSet("itv_users", "status=0,author='',authortime=0,meal=''", "where name=$id and (status=1 or status=999)");
        } 
...
if (isset($_POST['submitNotExpired'])) {
    if (empty($_POST['id'])) {
        ...
        foreach ($_POST['id'] as $id) {
            $db->mSet("itv_users", "status=999", "where name=$id and status=1");
...
if (isset($_POST['submitCancelNotExpired'])) {
    if (empty($_POST['id'])) {
        ...
        foreach ($_POST['id'] as $id) {
            $db->mSet("itv_users", "status=1", "where name=$id and status=999");
...
if (isset($_POST["s_meals"]) && isset($_POST["e_meals"])) {
    if (empty($_POST["s_meals"])) {
        ...
        foreach($_POST["id"]as $mealid => $userid) {
            $db->mSet("itv_users", "meal=" . $_POST["s_meals"], "where name='$userid'");
...
// 获取用户总数并根据每页显示数量计算页数
if ($row = $db->mGetRow("itv_users", "count(*)", "where status>-1")) {
    $userCount = $row[0];
