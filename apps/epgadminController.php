...
if (isset($_POST['clearbind'])) {
    ...
    $db->mSet("itv_epg", "content=NULL", $where);
...
if (isset($_POST['bindchannel'])) {
    $result = $db->mQuery("SELECT distinct name FROM itv_channels");
    ...
    if (isset($_POST['id'])) {
        ...
        $db->mSet("itv_epg", "content='$content'", "where id=" . $_POST['id']);
    ...
    $result = $db->mQuery("SELECT id,remarks,content from itv_epg where remarks != ''");
    if (!mysqli_num_rows($result)) {
        mysqli_free_result($result);
        ...
        foreach ($clist as $cname => $channel) {
            if (strstr($channel['name'], $row['remarks']) !== false) {
                $list[$cname] = $channel['name'];
                $content = implode(",", array_unique($list));
                $db->mSet("itv_epg", "content='$content'", "where id=" . $row['id']);
            } 
        } 
        unset($list);
    } 
...
if ($_GET["act"] == "online") {
    ...
    $db->mSet("itv_epg", "status=1", "where id=$id");
...
if ($_GET["act"] == "downline") {
    $id = !empty($_GET["id"])?$_GET["id"]:exit("<script>$.alert({title: '错误',content: '参数不能为空！',type: 'red',buttons: {confirm: {text: '确定',btnClass: 'btn-primary',action: function(){self.location=document.referrer;}}}});</script>");
    $db->mSet("itv_epg", "status=0", "where id=$id");
...
if ($_GET["act"] == "add") {
    ...
    $epg_name = $epg . '-' . $name;
    $result = $db->mQuery("select * from itv_epg where name=" . "'" . $epg_name . "'"); 
    // EPG是否已经同名或存在
    if (mysqli_num_rows($result)) {
        mysqli_free_result($result);
    ...
    // 新加EPG数据
    $db->mInt("itv_epg", "name,remarks", "'" . $epg_name . "','" . $remarks . "'");
...
if (isset($_POST['submitepgapi_chk'])) {
    ...
    $db->mSet("itv_config", "value='$tipepgerror_1000'", "where name='tipepgerror_1000'");
    $db->mSet("itv_config", "value='$tipepgerror_1001'", "where name='tipepgerror_1001'");
    $db->mSet("itv_config", "value='$tipepgerror_1002'", "where name='tipepgerror_1002'");
    $db->mSet("itv_config", "value='$tipepgerror_1003'", "where name='tipepgerror_1003'");
    $db->mSet("itv_config", "value='$tipepgerror_1004'", "where name='tipepgerror_1004'");
    $db->mSet("itv_config", "value='$tipepgerror_1005'", "where name='tipepgerror_1005'");
    $db->mSet("itv_config", "value='$epg_api_chk'", "where name='epg_api_chk'");
...
// 初始化数据
$epg_api_chk = $db->mGet("itv_config", "value", "where name='epg_api_chk'");
if ($epg_api_chk == 1) {
    $epg_api_chk = 'checked="checked"';
} else {
    $epg_api_chk = "";
} 
$tipepgerror_1000 = $db->mGet("itv_config", "value", "where name='tipepgerror_1000'");
$tipepgerror_1001 = $db->mGet("itv_config", "value", "where name='tipepgerror_1001'");
$tipepgerror_1002 = $db->mGet("itv_config", "value", "where name='tipepgerror_1002'");
$tipepgerror_1003 = $db->mGet("itv_config", "value", "where name='tipepgerror_1003'");
$tipepgerror_1004 = $db->mGet("itv_config", "value", "where name='tipepgerror_1004'");
$tipepgerror_1005 = $db->mGet("itv_config", "value", "where name='tipepgerror_1005'");
// 搜索关键字
...
if ($row = $db->mGetRow("itv_epg", "count(*)")) {
