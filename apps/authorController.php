...
if (isset($_POST['submitdel'])) {
    foreach ($_POST['id'] as $id) {
        $db->mDel("itv_users", "where name=$id");
...
if (isset($_POST['submitauthor'])) {
    ...
    if (empty($_POST['id']) || empty($_POST['meal_s'])) {
        ...
        foreach($_POST['id']as $userid => $id) {
            ...
            $db->mSet("itv_users", "status=1,exp=$exp,author='$administrator',authortime=$nowtime,marks='已授权',meal='$meal'", "where name='$id'");
        } 
        unset($meals);
...
if (isset($_POST['submitauthorforever'])) {
    ...
    if (empty($_POST['id']) || empty($_POST['meal_s'])) {
        ...
        foreach($_POST['id']as $userid => $id) {
            ...
            $db->mSet("itv_users", "status=999,exp=$exp,author='$administrator',authortime=$nowtime,marks='已授权',meal='$meal'", "where name='$id'");
        } 
        unset($meals);
...
if (isset($_POST['submitforbidden'])) {
    if (empty($_POST['id'])) {
        ...
        foreach ($_POST['id'] as $id) {
            ...
            $administrator = $_SESSION['user'];
            $db->mSet("itv_users", "status=0", "where name='$id'");
...
if (isset($_POST['submitdelonedaybefor'])) {
    ...
    $db->mDel("itv_users", "where status=-1 and lasttime<$onedaybefore");
    echo"<script>lightyear.notify('已删除一天前未授权用户！', 'success', 3000);</script>";
} 
// 删除所有未授权用户
if (isset($_POST['submitdelall'])) {
    $db->mDel("itv_users", "where status=-1 or status=-999");
...
// 获取用户总数并根据每页显示数量计算页数
if ($row = $db->mGetRow("itv_users", "count(*)", "where status=-1 or status=-999 or status=0")) {
    $userCount = $row[0];
...
if ($row = $db->mGetRow("itv_users", "count(*)", "where status=-1 or status=-999 and lasttime>$todayTime")) {
...
$todayTime = strtotime(date("Y-m-d"), time());
if ($row = $db->mGetRow("itv_users", "count(*)", "where lasttime>$todayTime")) {
    $todayuserCount = $row[0];
} else {
    $todayuserCount = 0;
} 
// 获取当天授权用户总数
if ($row = $db->mGetRow("itv_users", "count(*)", "where status>0 and authortime>$todayTime")) {
...
$isfreeuser = $db->mGet("itv_config", "value", "where name='trialdays'");
