                	...
                	<div class="table-responsive">
						<table class="table table-hover">
							...
							<tbody>
								<?php
								$num=1;
								$result=$db->mQuery("SELECT name from itv_category WHERE type='default' order by id");
								while ($row=mysqli_fetch_array($result)) {
									$categoryname=$row['name'];
									if ($channelnumdata = $db->mGetRow("itv_channels","count(*)","where category='$categoryname'")) {
