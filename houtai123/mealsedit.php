...
if ($_GET["act"]=="edits") {
    ...
    $db->mSet("itv_meals","name='$meal_name',content='$ids',amount='0',days='$days'","where id='$id'");
...
if ($_GET["act"]=="edit") { 
	...
	//检查套餐是否存在
	$result=$db->mQuery("SELECT name,content,amount,days FROM itv_meals WHERE id=".$id);
	...
	mysqli_free_result($result);
	//获取套餐所有的收视内容
	$result=$db->mQuery("SELECT id,name from itv_category where enable=1 ORDER BY id ASC");
	if (!mysqli_num_rows($result)) {
		mysqli_free_result($result);
