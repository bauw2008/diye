...
if ($_GET["act"] == "edits") {
    ...
    $db->mSet("itv_epg", "name='" . $epg_name . "',content='" . $ids . "',remarks='" . $remarks . "'", "where id=" . $id);
...
if ($_GET["act"]=="edit") { 
	...
	//检查EPG是否存在
	$result=$db->mQuery("select name,content,remarks from itv_epg where id=".$id);
	...
	}else if(strstr($row["name"],"TVTMS") != false){
		$epgname = substr($row["name"], 6);
		$epgopt = '<option value="TVTMS"  selected>TVTMS</option>';
	}
	unset($row);
	mysqli_free_result($result);
	//获取频道内容
	$result=$db->mQuery("SELECT distinct name FROM itv_channels order by category,id");
	if (!mysqli_num_rows($result)) {
		mysqli_free_result($result);
